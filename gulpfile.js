...
gulp.task('clean', () => del(['readonly_docs_theme', 'dist', 'build']));

// Theme
gulp.task('theme:files', () =>
    gulp.src('src/theme/{*.*,{static,templates}/**}')
        .pipe(gulp.dest('readonly_docs_theme'))
);
...
gulp.task('theme:scss', () =>
    gulp.src('src/theme/scss/readonlydocs.scss')
        .pipe(sass())
...
gulp.task('theme:js:worker', () =>
    gulp.src('src/theme/js/offline/worker.js')
        ...
        .pipe(gulp.dest('readonly_docs_theme/extra'))
);
...
gulp.task('theme:js:gettext', ['theme:files'],
    shell('babel', 'python', ['setup.py', 'extract_messages', '-o', 'readonly_docs_theme/theme.pot'])
);
...
gulp.task('homepage:load-data', () =>
    !renderData && require('./src/homepage/data/readonlydocs').loadData()
...
gulp.task('homepage:scss', () =>
    gulp.src('src/homepage/scss/readonlydocs.scss')
        .pipe(sass())
        .pipe(postcss([autoprefixer()]))
        .pipe(cleanCSS())
